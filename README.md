...
系统已经部署到生产环境：[https://82.156.210.199/](https://82.156.210.199/)。仓库为私有项目，常规发布只允许来自已验证的 `main` 和 GitHub Release；本次 `v0.6.15` 按项目所有者授权继续采用本地 exact-commit 路径，不同步 GitHub。生产数据库、录音、微信状态、密钥和备份不进入 Git。

## 代码、发布与生产状态

| 项目 | 状态 |
| --- | --- |
| 当前开发候选 | `v0.6.15`（小小微信图片记账、自然语言确认、压缩凭证与发票自动匹配本地候选） |
| v0.6.15 发布方式 | 不同步 GitHub；本地完整门禁、注释标签、不可变归档、manifest、SHA-256 与生产 evidence |
...
| 本次发布前生产源码版本 | `v0.6.14` |
| 本次发布前生产提交 | `e124c7a501a02a47ed6e2f24ffe799cbbcecd762` |
...
| 本次发布前生产 release | `/opt/sentelligent-sales-workbench/releases/v0.6.14-20260824T123943Z_e124c7a501a0` |
| 本次回滚基线 | v0.6.14 不可变 release；切换脚本还会生成 fresh 数据库和微信会话备份 |
| v0.6.1 历史生产状态 | 当时切换前后预检均 `25/25`，HTTPS smoke `25/25`、`cleanup=clean`，数据库完整性通过 |
| 发布前生产代码 | `e124c7a501a0` 已运行；其旧预检不能替代 v0.6.15 的 fresh 证据 |
...
上述历史现网字段不能替代本次 v0.6.15 的 fresh 生产证据；切换前必须重新采集数据库、服务、主机身份和受保护服务快照。生产部署细节见 [部署记录](docs/部署记录.md)，本次候选边界见 [v0.6.15 版本说明](docs/releases/v0.6.15.md)。
...
| 小小微信记账 | 本人私聊发送付款凭证图片或收入/支出文字后，OCR/AI 生成固定格式草稿；交易列表图片可拆成多笔，引用草稿后可用自然语言确认、修改或取消 | 未确认不写账；发生日期不明时保持待确认；可报销金额等于消费金额，借款到账作为收入并按自然周处理 |
...
- 生产默认只部署已合并到 `main` 且已打标签的提交；本次 `v0.6.15` 是不推 GitHub 的明确授权例外，仍必须从干净 final exact commit 和唯一注释标签生成制品。
...
候选版本的 vendored `weixin-agent-sdk@0.5.0-sentelligent.5` worker 通过独立机器 Token 调用：
