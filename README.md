...
| 项目 | 状态 |
| --- | --- |
| 当前开发候选 | `v0.6.3`（统一助手、森特本地快捷记账复核、医院招标轮巡；候选验收中） |
...
| iCost/快捷记账配置 | 系统配置页生成/轮换森特本地记账 Token；iOS 快捷指令提交出差报销后由微信小小助手复核，确认后写入森特费用和付款记录 | 森特与轻氧完全割裂；当前候选不发送远端账本，也不接受收入记账 |
| 自有 iOS 快捷指令 | 一个账号 Token、一次森特出差报销分类选择、截图文字上传；识别草稿先经微信小小助手复核，确认后写入森特差旅费用和付款记录 | v0.6.3 候选开发中；出差报销收入暂不开放，确认前不会创建财务记录 |
...
| 医院招标监测 | 公开来源采集、每小时一批 10 客户自动轮巡、持久化游标/快照/锁、来源健康、匹配证据和 PushPlus 聚合通知 | v0.6.3 候选已完成本地合成验收；生产需观察首轮真实采集与通知，不依赖额外招标 API，不自动修改客户或商机 |
