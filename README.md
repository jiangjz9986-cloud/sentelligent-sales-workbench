...
| 当前开发目标 | `v0.4.3`，分支 `codex/v0.4.3-centos-cutover-compat` |
| `v0.4.3` 状态 | 基于已发布的 `v0.4.2`，修复 CentOS 7 对新 systemd 属性单项查询及 systemd 219 缺少 `DynamicUser` 字段的不兼容；业务 API、数据库迁移和用户工作流不变，正式状态以新 tag、GitHub Release 和生产证据为准 |

上述现网版本、release 路径、服务状态和健康接口已于 `2026-08-08` 只读复核。生产部署细节见 [部署记录](docs/部署记录.md)，本次候选说明见 [v0.4.3 版本说明](docs/releases/v0.4.3.md)。
