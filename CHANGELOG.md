...
- 住宿费用按识别出的区域和晚数自动生成备注；其他费用仅在用途明确时生成备注，商户名不会被误当作费用事由。日期、城市和非实际计划区域的识别规则同步收紧。
- 微信小小记账可按账号新增、重命名、管理识别关键词、停用自定义收入/支出分类；关键词用于图片与自然语言记账分类，旧客户端未提供关键词时保持兼容。
- 发票入库或复核通过后自动尝试关联已存在记账；仅唯一、金额和日期均合规的匹配会自动确认，歧义继续待人工复核。
- 周替票建议按发票可用余额将单张发票分摊到多笔手动选为替票的费用，并保持建议与批量确认的金额上限。界面摘要明确区分发票张数和分摊笔数。
- 新增迁移 `0050_bookkeeping_category_aliases`，仅为分类表增加空关键词列，不改写已有分类或账本数据。

## [0.13.14] - 2026-09-24

### 记账费用分类管理

- 新增按账号隔离的收入/支出费用分类目录，支持新增、编辑、停用、恢复和查看归档分类。
- 系统默认分类保留稳定的历史解析能力；系统分类名称不可改，子分类可按实际业务调整，自定义分类可完整维护。
- 记账写入、微信确认和历史修订统一使用当前账号的分类目录；停用分类不再用于新记账，但历史记录仍可安全读取和修订。
- 分类写入使用乐观锁、审计记录和唯一性校验，避免并发修改覆盖或跨账号读取。
- 新增数据库迁移 `0049_bookkeeping_categories`，仅创建分类目录与索引，不改写既有记账数据。

## [0.13.13] - 2026-09-23

### 记账后反向匹配已入库发票

- 发票先入库、记账后创建时，系统按金额、付款/开票日期顺序、自然周优先和 31 天窗口自动检索并绑定唯一合规发票。
- 同额多张、日期不合规、超出日期窗口或替票记账继续停留人工复核，不自动猜测；已有的发票先入账匹配规则保持一致。
- 微信确认回执在自动绑定成功时只追加“已自动关联发票”，批量回执仍在最后一笔完成后一次性发送。
- 不新增数据库迁移，不改写既有发票或费用数据。

## [0.13.12] - 2026-09-22

### 发票入库回执与替票匹配规则修复

- 微信发票入库回执只展示本次发票的发票号、开票日、销售方和金额，不再暴露内部仓库编号、候选费用编号和自动匹配过程。
- 重复上传时返回已在仓库的发票识别摘要；识别字段缺失时明确提示待补充。
- 明确标记为替票的费用不再参与自动发票匹配，继续由发票仓库和人工选择流程处理。
- 不新增数据库迁移，不改写既有发票或费用数据。
...
- 最近微信记账回执属于账本辅助信息；若历史回执关联的费用或付款已失效，跳过该条异常回执，不再阻断整周账本读取。
- 直接读取单条异常回执时仍保留冲突保护；首次周账本加载失败时明确显示“同步失败”，409 提示附带错误码和请求编号。
- 不新增数据库迁移；不会自动改写生产已有账本数据。

## [0.13.10] - 2026-09-20

### 差旅记账删除与票据关联判定

- 修复未手动选择发票/替票的记账仍可能因自动候选、无票确认或凭证导入历史而无法删除的问题。
- 仅手动确认的发票匹配和用户已接受的替票候选继续阻止删除；删除时撤销自动匹配、过期未接受候选并撤销无票确认。
- 保留导入与凭证来源历史，向前端返回具体阻塞关联并显示明确提示；不新增数据库迁移。

## [0.13.9] - 2026-09-20

### 差旅记账关联票据删除恢复

- 删除费用被发票匹配、替票、无票确认或待处理凭证阻止时，明确说明记录和发票原件均保留。
- 将确认按钮改为前往发票管理处理关联，避免用户重复提交不会成功的删除请求。
- 增加自动 Chrome 回归并纳入正式集成 QA；不新增数据库迁移。

## [0.13.8] - 2026-09-19

### 差旅记账删除与发票仓库周替票

- 修复费用账本删除按钮：使用站内确认弹窗，取消不改数据，确认后刷新并移除记录；依赖冲突明确反馈。
- 发票仓库按自然周汇总缺票金额、手动标记的替票缺口、候选覆盖及未覆盖金额。
- 按费用类别、付款与开票日期和发票可用余额为替票缺口生成发票组合建议；一组多张发票可在复核后按周事务性确认。
- 候选仅为建议，不会自动确认或占用发票；冲突、幂等和审计沿用服务端写入边界。
- 不新增数据库迁移。
...
- 编辑已有记账时隐藏其他费用字段；新建记账仍保留完整字段，编辑保存继续保留隐藏数据。
- 缩短付款凭证预览，移除重复发票状态摘要；当前发票选项统一使用浅绿色选中态。
- 桌面编辑卡片按内容收高并限制最大高度，较长内容保留卡片内滚动；窄屏继续使用全屏布局。
- 不新增数据库迁移。
...
- 将列表行详情和编辑收敛到同一个右侧卡片；查看、编辑、取消和保存都在同一层内切换。
- 详情与编辑共用付款凭证和发票区域，缩小凭证空白，保留清晰预览、单击查看原件、双击替换图片及凭证删除。
- 发票状态使用互斥选项展示电子、替票、纸质和未提供，新增记账默认未提供。
- 编辑主视图聚焦支付时间、出差区域、付款金额和费用事由；其他字段收纳在可折叠区域。

## [0.13.5] - 2026-09-19

### 差旅费用详情卡与付款凭证替换

- 将记账详情卡收口为“详情内容”，按付款逐笔展示支付时间、出差区域、付款金额和费用事由。
- 将付款凭证与发票状态合并到详情卡双栏区域，发票状态区分电子、替票、纸质和未提供四种状态。
- 支持在详情卡中单击查看凭证原件、双击图片选择本地文件替换，并通过版本校验和审计记录保证附件替换可追溯。
