...
function defaultRunner({ command, args, timeoutMs, signal }) {
  return new Promise((resolve, reject) => {
    if (signal?.aborted) return reject(rendererError("PDF_IMAGE_RENDER_CANCELLED"));
    let settled = false;
    let stopError = null;
    ...
    const abort = () => terminate(rendererError("PDF_IMAGE_RENDER_CANCELLED"));
    const timer = setTimeout(() => terminate(rendererError("PDF_IMAGE_RENDER_TIMEOUT")), timeoutMs);
    ...
    child.once("close", (code) => {
      if (stopError) finish(reject, stopError);
      else if (code !== 0) finish(reject, rendererError("PDF_IMAGE_RENDER_FAILED"));
      else finish(resolve);
    });
  });
}
...
export function createLocalPdfImageRenderer(options = {}) {
  ...
  return {
    async render(value, { signal } = {}) {
      if (signal?.aborted) throw rendererError("PDF_IMAGE_RENDER_CANCELLED");
