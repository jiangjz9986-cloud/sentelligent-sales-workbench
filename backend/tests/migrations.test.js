...
test("records versioned migrations exactly once and remains idempotent on reopen", () => {
  withDatabase((databaseUrl) => {
    ...
    try {
      ...
      assert.equal(firstMigrations.length, 46);
...
test("current migrations upgrade a complete 0032 database through 0033-0047 in order", () => {
  withDatabase((databaseUrl) => {
    const db = openDatabase({ databaseUrl });
    try {
      ...
      assert.equal(ledgerAfter.length, 46);
  ...
  withDatabase((databaseUrl) => {
    ...
    try {
      ...
      assert.deepEqual(
        ...
        ["0001", "0002", "0003", "0005", "0006", "0007", "0008", "0009", "0010", "0011", "0012", "0013", "0014", "0015", "0016", "0017", "0018", "0019", "0020", "0021", "0022", "0023", "0024", "0025", "0026", "0027", "0028", "0029", "0030", "0031", "0032", "0033", "0034", "0035", "0036", "0037", "0038", "0039", "0040", "0041", "0042", "0043", "0044", "0045", "0046", "0047"],
      );
    } finally {
      migrated.close();
    }
...
test("adopts legacy baseline tables by adding missing columns without losing rows", () => {
  withDatabase((databaseUrl) => {
    ...
    try {
      ...
      assert.equal(all(db, "SELECT version FROM schema_migrations").length, 46);
    } finally {
      db.close();
    }
  });
