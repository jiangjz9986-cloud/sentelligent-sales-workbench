...
test("exact v0.10.3 remains forward-compatible with the current database", async () => {
  ...
  try {
    ...
    // Phase 2: current code upgrades the exact 0032 database through 0033-0049, then its repository/API writes the new ASR key and an owner-scoped
    // customer fixture.
    ...
    try {
      ...
      assert.equal(versions.length, 48);
      assert.deepEqual(versions.slice(-17), [
