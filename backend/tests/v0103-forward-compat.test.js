...
test("exact v0.10.3 remains forward-compatible with the current database", async () => {
  ...
  try {
    ...
    // Phase 2: current code upgrades the exact 0032 database through 0033-0050, then its repository/API writes the new ASR key and an owner-scoped
    // customer fixture.
    ...
    try {
      ...
      assert.equal(versions.length, 49);
      assert.deepEqual(versions.slice(-18), [
    ...
    // Phase 3: boot exact release code against the current migrated database and exercise
