...
test("explicit recovery requeues only transient delivery failures without resetting attempts", () => {
  withDatabase((db) => {
    ...
    const repository = createWeixinConfirmationOutboxRepository(db, {
      clock: clock.now,
      maxAttempts: 1,
      idFactory: () => "outbox-requeue",
    });
    const created = repository.enqueue({
      owner: "owner-1",
      conversationId: "conversation-1",
      idempotencyKey: "shortcut-requeue",
      payload: { actionId: "action-1", entryId: "entry-1", version: 1, kind: "confirmation" },
    });
    const lease = repository.leaseNext({ renderMessage: () => "draft" });
    const failed = repository.ackFailure(created.id, {
    ...
    const requeued = repository.requeueFailed(created.id);
...
test("explicit recovery refuses scope and lifecycle terminal failures", () => {
  withDatabase((db) => {
    const repository = createWeixinConfirmationOutboxRepository(db, {
      ...
      idFactory: (() => {
        let sequence = 0;
        return () => `outbox-terminal-${++sequence}`;
      })(),
    });
    const scoped = repository.enqueue({
      owner: "owner-1",
      conversationId: "conversation-1",
      idempotencyKey: "shortcut-terminal-scope",
      payload: { actionId: "action-1", entryId: "entry-1", version: 1, kind: "confirmation" },
    });
    const scopedLease = repository.leaseNext({ renderMessage: () => "draft" });
    repository.discardLeased(scoped.id, {
      leaseToken: scopedLease.leaseToken,
      errorCode: "WEIXIN_DELIVERY_SCOPE_MISMATCH",
    });
    assert.throws(
      () => repository.requeueFailed(scoped.id),
      (error) => error?.code === "WEIXIN_OUTBOX_NOT_RETRYABLE",
    );

    const superseded = repository.enqueue({
      owner: "owner-1",
      conversationId: "conversation-1",
      idempotencyKey: "shortcut-terminal-superseded",
      payload: { actionId: "action-2", entryId: "entry-2", version: 1, kind: "confirmation" },
    });
    repository.closePending({
      owner: "owner-1",
      conversationId: "conversation-1",
      actionId: "action-2",
      entryId: "entry-2",
      errorCode: "WEIXIN_OUTBOX_SUPERSEDED",
    });
    assert.throws(
      () => repository.requeueFailed(superseded.id),
      (error) => error?.code === "WEIXIN_OUTBOX_NOT_RETRYABLE",
    );
  });
});

test("closes queued or leased draft messages when a newer decision supersedes them", () => {
  withDatabase((db) => {
    const clock = makeClock();
    const repository = createWeixinConfirmationOutboxRepository(db, {
      clock: clock.now,
      idFactory: (() => {
        let sequence = 0;
        return () => `outbox-supersede-${++sequence}`;
      })(),
    });
    repository.enqueue({
      owner: "owner-1",
      conversationId: "conversation-1",
      idempotencyKey: "shortcut-stale-1",
      payload: { actionId: "action-1", entryId: "entry-1", version: 1, kind: "confirmation" },
    });
    const closedQueued = repository.closePending({
      owner: "owner-1",
      conversationId: "conversation-1",
      actionId: "action-1",
      entryId: "entry-1",
    });
    assert.equal(closedQueued.closedCount, 1);
    assert.equal(repository.leaseNext({ renderMessage: () => "must not send" }), null);

    repository.enqueue({
      owner: "owner-1",
      conversationId: "conversation-1",
      idempotencyKey: "shortcut-stale-2",
      payload: { actionId: "action-1", entryId: "entry-1", version: 2, kind: "correction" },
    });
    const lease = repository.leaseNext({ renderMessage: () => "old draft" });
    assert.ok(lease);
    const closedLeased = repository.closePending({
      owner: "owner-1",
      conversationId: "conversation-1",
      actionId: "action-1",
      entryId: "entry-1",
    ...
    assert.equal(closedLeased.closedCount, 1);
    assert.equal(repository.get(lease.item.id).status, "failed");
    assert.equal(repository.get(lease.item.id).lastErrorCode, "WEIXIN_OUTBOX_CANCELLED");
    assert.throws(
      () => repository.requeueFailed(lease.item.id),
      (error) => error?.code === "WEIXIN_OUTBOX_NOT_RETRYABLE",
    );
    assert.throws(() => repository.ackSuccess(lease.item.id, { leaseToken: lease.leaseToken }), /lease|processing|current/i);
  });
});
