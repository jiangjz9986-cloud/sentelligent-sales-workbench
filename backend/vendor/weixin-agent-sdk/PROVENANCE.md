...
Fork version: 0.5.0-sentelligent.3
Allowed modified files: dist/index.mjs, dist/index.d.mts, package.json, PROVENANCE.md
Purpose: expose bounded inbound delivery metadata, derive opaque delivery identity, remove unsafe production logging/debug commands, enforce host authorization plus bounded inbound media handling, and support restart-safe proactive delivery through AES-256-GCM encrypted 23-hour context-token persistence bound to account, recipient, expiry, and host delivery key, with credential-free readiness and exact delivery-target checks.
