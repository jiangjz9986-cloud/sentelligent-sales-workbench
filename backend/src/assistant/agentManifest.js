...
export const AGENT_MANIFESTS = deepFreeze([
  ...
  manifestDefinition("payment-proof", {
    contractVersion: "payment-proof-v1",
    modelPolicy: "required_with_deterministic_fallback",
    taskTypes: ["ingest", "recognize", "candidate_match", "review"],
    tools: ["payment-proof.ingest"],
    confirmation: { preview: "preview", write: "explicit" },
    sourcePolicy: { mode: "required", requiredFields: ["sourceRefs", "document"] },
    inputSchema: {
      type: "object",
      required: ["taskType", "document"],
      properties: { taskType: "enum", document: "object" },
    },
    outputSchema: {
      type: "object",
      required: ["schemaVersion", "status", "facts", "unknowns", "sourceRefs", "writebackPreview"],
      properties: { schemaVersion: "payment-proof-v1", document: "object", recognition: "object", candidateMatch: "object" },
    },
    systemPrompt: [
      "你是森特智行实付凭证 Agent。",
      "原始图片或 PDF 必须由服务端无损处理；Agent 运行记录不得保存原件、文件路径或 OCR 全文。",
      "识别字段只是待复核证据，候选付款必须由 owner-scoped 服务端重新校验，不得自动关联。",
      "入待处理区使用现有原子写入；正式关联、拒绝或删除必须本人确认。",
    ].join(""),
    fallback: { strategy: "return a redacted review-required recognition preview", status: "fallback" },
  }),
  manifestDefinition("invoice", {
    contractVersion: "invoice-v1",
    modelPolicy: "required_with_deterministic_fallback",
    ...
    outputSchema: {
      type: "object",
      required: ["schemaVersion", "status", "facts", "unknowns", "sourceRefs", "writebackPreview"],
      properties: { schemaVersion: "invoice-v1", document: "object", recognition: "object", matchPreview: "object" },
    },
    systemPrompt: [
      "你是森特智行发票 Agent。",
      "原始发票必须由服务端无损处理；Agent 运行记录不得保存原件、文件路径、OCR 全文或模型原始载荷。",
      "识别字段只能作为待复核事实，不能自动匹配费用或替代无票确认。",
      "原件入库使用现有原子写入；匹配、替代无票确认和删除必须本人确认。",
    ].join(""),
  ...
  manifestDefinition("advance-settlement", {
    ...
    systemPrompt: [
      ...
      "输出始终是待人工确认预览；不得创建退款/补款流水，不得修改费用、请款金额或状态，也不得声称交易已经发生。",
    ].join(""),
  ...
  manifestDefinition("reimbursement-report", {
    ...
    outputSchema: {
      type: "object",
      required: ["schemaVersion", "status", "facts", "unknowns", "sourceRefs", "writebackPreview"],
      properties: { schemaVersion: "reimbursement-report-v1", weekStart: "date", summary: "object", invoiceCoverage: "object", printReadiness: "object" },
    },
    systemPrompt: [
      "你是森特智行报销周汇总 Agent。",
      "只使用 owner-scoped 自然周费用快照，保留实付、可报销、发票状态和来源引用。",
      "金额异常必须标记人工核对，不得修改费用、补齐发票或推断公司规则。",
      "输出始终是只读预览，不得声称已保存、已打印、已发布或已提交报销。",
    ].join(""),
    fallback: { strategy: "return deterministic source-backed reimbursement summary and blockers", status: "fallback" },
  }),
