...
function recordHeadline(record) {
  ...
  return `${weixinShortId(record.id)}  ${date ?? "日期待确认"}  ${customer}`;
}
  ...
  return Object.freeze({
    async "visit-capture.capture"({ arguments: argumentsValue, context, businessContext, serverData }) {
      ...
      const opportunityLine = opportunityCandidate?.id
        ? opportunityCandidate.name
        : "待确认";
      const candidateChecks = [
        customerCandidate?.status === "ambiguous" ? "客户候选不唯一" : null,
        opportunityCandidate?.status === "ambiguous" ? "商机候选不唯一" : null,
        opportunityCandidate?.status === "conflict" ? "客户与商机关系不一致" : null,
      ].filter(Boolean).join("；");
      const previewText = weixinCard("小小提醒！新增一条拜访记录", [
        ...(degraded ? [["AI 状态", "暂不可用，已按原文整理"]] : [["AI 状态", "已识别，待你确认"]]),
        ["时间", occurredAt ? shanghaiDateOf(occurredAt) : "现在"],
        ["客户", customerLine],
        ["商机", opportunityLine],
        ["诉求", summaryText(analysis, "request", weixinClip(rawContent, 80, "待补充"))],
        ["风险", summaryText(analysis, "risk", "无")],
        ["建议", summaryText(analysis, "action", "待确认")],
        ...(candidateChecks ? [["校验", candidateChecks]] : []),
      ]);
      return {
    ...
    async "visit-capture.update"({ arguments: argumentsValue, context, serverData }) {
      ...
      if (field === "occurredAt") {
        ...
        lines.push(["客户", `${record.customerName ?? "未挂客户"} → ${resolved.customer.name}`]);
        ...
        changes = { fields };
        lines.push(["商机", `${record.opportunityId ? weixinShortId(record.opportunityId) : "未挂商机"} → ${resolved.opportunity.name}`]);
        ...
        lines.push([SUMMARY_FIELD_LABELS[key], `${excerpt(before || "空", SUMMARY_TEXT_MAX)} → ${weixinClip(value, SUMMARY_TEXT_MAX)}`]);
      } else {
        return block("暂不支持修改该字段。可改：发生时间/客户/商机/诉求/反馈/风险/建议动作。");
      }
      const previewText = weixinCard("小小提醒！修改拜访记录", lines);
      return {
        arguments: {
    ...
    async "visit-capture.void"({ arguments: argumentsValue, context, serverData }) {
      ...
      const previewText = weixinCard("小小提醒！作废拜访记录", [
        ["编号", weixinShortId(record.id)],
        ["原文", excerpt(record.rawContent)],
        ["状态", record.status === "confirmed" ? `${statusLabel}（已写回内容不回退）` : statusLabel],
      ]);
      return {
        arguments: {
          quickRecordId: record.id,
