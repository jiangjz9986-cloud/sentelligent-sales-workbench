...
function promptFor(documentKind, referenceDate) {
  const base = DOCUMENT_PROMPTS[documentKind];
  if (!base || documentKind !== "payment_proof" || !referenceDate) return base;
  return [
    base,
    `参考日期是 ${referenceDate}（Asia/Shanghai），只用于补齐凭证上已经清晰出现的月日所缺少的年份。`,
    "凭证有完整年月日时按凭证；只有月日时，选择不晚于参考日期的最近一个对应日期；凭证没有日期时仍返回 null，不能用参考日期代替。",
  ].join("\n");
}
...
export async function analyzeDocumentWithVision(file, options = {}) {
  ...
  try {
    ...
    return await completionObject(response);
  } catch (error) {
    if (error instanceof DocumentVisionError) throw error;
