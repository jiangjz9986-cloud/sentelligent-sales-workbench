...
这是完全绕过 iCost 的兼容版 V4（当前修复文件名为“自有截图记账（兼容版V4修复）”）。森特 v0.6.3 候选开放 `POST /api/integrations/shortcut/bookkeeping`：有效账号 Token 且森特本地微信复核配置完整时，验证响应返回 `bookkeepingReady=true`，快捷指令继续截图、提取文字并提交一次分类记账请求。

账本与分类目录固化在 `build-bookkeeping-shortcut.mjs` 的 `BOOKKEEPING_CATALOG` 中，并编译为 12 个合法显示项，例如“出差报销 · 支出 · 交通 · 打车”。当前只保留森特出差报销支出；用户只选择一次，不会重复选择账本；子分类没有值时显示“无”。后端按同一目录解析 `selection_path` 并再次校验，避免写入错误分类。
...
Apple 的导入配置页只做参数替换，完成添加前不会运行网络动作，因此 Token 验证不能控制系统的“添加快捷指令”按钮。V4 会在成功安装后的每次运行开头调用 `GET /api/integrations/shortcut/verify`；接口只有在 Token 有效且森特本地微信复核配置完整时才返回 `bookkeepingReady=true`。验证本身不会调用 AI 或创建账目，只会更新 Token 的 `last_used_at` 元数据。
...
目录接口只提供 `出差报销 → sentelligent`，首期只开放支出。经服务端分析后先进入 `review_required`，并在森特网页“差旅费用 → 付款凭证 → 快捷指令待复核”显示 owner-scoped 记录；微信小小助手会发送识别草稿，支持补齐日期/金额/用途/商户/分类/子分类/备注，只有最新确认码通过后才创建正式费用和付款记录。出差报销收入分类显示为空并由 API 拒绝，不伪造不存在的收入模型。

同一账号的 `idempotency_key` 全局唯一。V4 的幂等键原文只由 OCR 文本、分类和备注组成，不含 `CurrentDate` 等每次运行都会变化的值；整条快捷指令在响应丢失后重跑仍会复用同一 key。微信发送失败、确认码冲突、状态不完整或处理超时都 fail closed；outbox 使用持久化租约和固定错误码重试。用户账号 Token 只发送给森特，永远不会转发到其他系统。
