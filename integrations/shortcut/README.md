...
当前智能截图轻量流程以设备直连 V5 为准；手动账号密码、三级菜单 V9 与账号配对 V7 仅保留为兼容路径。所有流程都不使用六位确认码。

## V5（旧 iCost 截图流程、设备直连）

V5 原样保留用户旧 iCost 快捷指令的截图、裁剪和 OCR，只把最后的 iCost 写入替换为森特智行 HTTPS 提交。请求体仅包含 OCR 纯文本与固定来源，快捷指令内没有账号、密码、菜单或时间 ID 动作；设备身份只存在于用户个人签名副本。

服务器对规范化 OCR 文本生成带域隔离的 SHA-256 防重复键。同一张截图重试只返回同一待确认记录，不会重复建单；不同截图使用不同键。提交仍只创建微信待确认草稿，只有用户向小小精确回复“确认”才写账。
