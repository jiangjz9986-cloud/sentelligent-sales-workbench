...
test("migration schema and checksum ledger roll back together on failure", () => {
  ...
  try {
    ...
    assert.equal(db.prepare("SELECT count(*) n FROM platform_migrations").get().n, 11);
    assert.equal(db.prepare("SELECT count(*) n FROM sqlite_schema WHERE name = 'provider_readiness_evidence'").get().n, 1);
    assert.equal(db.prepare("SELECT admission_mode FROM tasks LIMIT 0").columns()[0].name, "admission_mode");
