...
本提交交付的是独立平台底座、管理 API、静态管理台、业务侧受限客户端和运维脚本。它不是现有业务系统的最终切换提交，也没有改动另一升级工作树、业务数据库或生产进程。
...
1. 与原升级任务确认共享文件整合窗口及其已提交 SHA，只在本工作树串行摘取/合并明确提交。
2. 按 `CONTRACT.md` 为每个文本入口建立业务快照、权限、结果卡和幂等适配测试，再逐项启用平台调用。
3. 迁移主动调度所有权前先完成停旧、排空、启新、回滚演练，确保不双跑、不重复写回、不重复通知。
