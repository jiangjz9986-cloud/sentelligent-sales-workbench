...
候选使用 vendored `weixin-agent-sdk@0.5.0-sentelligent.4` 的受限入站元数据调用：
...
快捷记账使用独立确认策略，不适用上述通用六码：只有精确回复 `确认`、以 `修改` 开头说明字段，或精确回复 `取消` 才会处理；六码和其他肯定表达不会写账。只有小小已经成功送达的当前版本草稿可以确认。
