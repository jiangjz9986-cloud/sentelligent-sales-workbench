...
Information architecture is fixed at nine primary modules. Customer profile owns customer records and tender monitoring; opportunity owns records, risk recognition, next actions, and Kanban; system settings owns AI security, WeChat binding, WeChat Clawbot notification status, and tender scheduling. Keep tender business results under customer profile, scheduler controls under system settings, and notification settings read-only.
...
Travel-expense rows open one right-side card in read-only detail mode. The edit action switches that same card in place; cancel returns to the unchanged detail, and save returns to the updated detail without opening a second layer. Keep payment proof and invoice status side by side, show the proof as a clear full-width rectangle without a duplicate view/count row, and keep invoice choices exclusive in edit mode.
