...
2. 三类 AI 入口都使用 OpenAI-compatible `POST /chat/completions`，模型名为 `deepseek-flash`。
...
- Backend WSL `npm run smoke`: passed with `aiAnalysisMode=model`, `modelProvider=deepseek`, `modelName=deepseek-flash`, `modelReady=true`.
